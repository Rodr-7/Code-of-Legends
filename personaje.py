        ...
        if self.muerte():
            ...
            if enemigo.muerte():
                ...
                if enemigo.proteger:# ----------------- Definicion de daño si el enemigo se protege
                    ...
                    print(f"{enemigo.nombre} se esta protegiendo.\n") 
                if danio > 0:
                    enemigo.vida -= danio
                    print(f"{self.nombre} ha realizado {danio} puntos de daño a {enemigo.nombre}.\n")
                    if enemigo.muerte():
                        enemigo.vida = 0
        ...
        if not fila.empty:
            ...
            try:
                ...
                if fila['Arma'] != "Palo": #--------------------- RESTAURANDO ARMA PREVIAMENTE EQUIPADA
                    ...
                    df = pd.read_csv(csv_path_arm)  # Leer el archivo CSV
...
#personaje1 = leer_personaje("Cloud")
#print(personaje1)
#print(personaje1.arma.categoria)
##############################################################################################################

#----------------------------------------------------  PRUEBAS  ---------------------------------------------------------------------------------------------------------------------------------
#personaje1= Caballero("Pedrito_csv")
#print(personaje1)
